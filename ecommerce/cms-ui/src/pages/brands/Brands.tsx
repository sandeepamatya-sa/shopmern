...
export const Brands = () => {
  ...
  const load = () => { setLoading(true); http.get('/cms/brands').then(r => setData(r.data)).catch(() => {}).finally(() => setLoading(false)) }
  useEffect(() => { load() }, [])
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      try {
        if (editing) { await http.put('/cms/brands/' + editing._id, values); toast.success('Brand updated!') }
        else { await http.post('/cms/brands', values); toast.success('Brand created!') }
  ...
  const handleEdit = (row: any) => { setEditing(row); formik.setValues({ name: row.name, status: row.status }); setShowForm(true) }
  const handleDelete = async () => {
    try { await http.delete('/cms/brands/' + deleteId); toast.success('Deleted!'); setDeleteId(null); load() }
    catch (err: any) { toast.error(err.response?.data?.message || 'Error') }
  }

  return (
    <>
      <PageHeader title="Brands" extra={
      ...
      {showForm && (
        <div className="card border-0 shadow-sm mb-4">
          <div className="card-body">
            <h6 className="fw-bold mb-3">{editing ? 'Edit Brand' : 'New Brand'}</h6>
            <form onSubmit={formik.handleSubmit} className="row g-3 align-items-end">
              <div className="col-md-4"><FormField formik={formik} name="name" label="Brand Name" placeholder="e.g. Apple" /></div>
      ...
      <ConfirmDelete show={!!deleteId} onConfirm={handleDelete} onCancel={() => setDeleteId(null)} name="brand" />
    </>
  )
}
